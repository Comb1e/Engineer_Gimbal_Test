...
void KalmanFilter_Init(KALMAN_1 *cfg) {
    cfg->x = 0;
    cfg->P = 5;
    cfg->A = 1;
    cfg->B = 0.01f;
...
float KalmanFilter_Update(KALMAN_1 *cfg, float measure, float u) {
    cfg->x = cfg->A * cfg->x + cfg->B * u;
    cfg->P = cfg->A * cfg->A * cfg->P + cfg->Q;
    cfg->K = cfg->P * cfg->H / (cfg->P * cfg->H * cfg->H + cfg->R);
    cfg->x = cfg->x + cfg->K * (measure - cfg->H * cfg->x);
    cfg->P = (1 - cfg->K * cfg->H) * cfg->P;
    return cfg->x;
}