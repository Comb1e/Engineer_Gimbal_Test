...
void autoCtrlTask(void *argument){
    ...
    for(;;) {
        ...
        g_arm.is_all_peripheral_connect = g_arm.framework.is_all_motors_safe
                                    && g_arm.actuator.is_all_motors_safe
                                    && !g_arm.actuator.arm_roll_IMU.is_imu_lost();
...
#if AUTO_REVIVE
        if(g_arm.is_enable && g_arm.is_none_of_peripheral_connect){// 这个其实可以用板子复位来做  NO!
            g_arm.set_arm_reset();
        }
#endif
        stat = osSemaphoreAcquire(g_arm.communication.can.can_more_dev.rx.semaphore,5);//这里只是检查basic，没有检查其它包
        if(osOK==stat){
            g_arm.set_communication_more_data_to_rx_data();
        }
        osDelay(1);
    }
}